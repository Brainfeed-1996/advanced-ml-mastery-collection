# Q-Learning - Maze Solver

**Folder:** `06-Reinforcement-Learning`

**Last updated:** 2026-02-16 16:29:34

## Purpose

Solve mazes using Q-Learning algorithm

## Key Features

- Maze environment creation
- Q-Learning implementation
- Exploration vs exploitation
- Policy visualization
- Solution evaluation

## How to Use

Run the notebook to train a maze solver. Adjust learning parameters.

## Expected Outputs

Trained Q-table, solution path, convergence plots

## Difficulty Level

- **Intermediate**

## Prerequisites

- Python 3.10+
- Virtual environment (see docs/INSTALLATION.md)
- Required dependencies (see docs/OPTIONAL_HEAVY_DEPS.md)

## Running the Notebook

### Interactive
```bash
jupyter notebook 06-Reinforcement-Learning/Q-Learning-Maze-Solver.ipynb
```

### Headless (with outputs)
```bash
python -m jupyter nbconvert --to notebook --execute \
  06-Reinforcement-Learning/Q-Learning-Maze-Solver.ipynb --output Q-Learning-Maze-Solver.ipynb --output-dir 06-Reinforcement-Learning
```

## Troubleshooting

- **Missing outputs**: Run the notebook in Jupyter or use nbconvert
- **Memory issues**: Reduce batch size or use smaller models
- **Slow execution**: Use CPU-safe defaults or reduce complexity
- **Dependency errors**: Install required packages from requirements.txt
